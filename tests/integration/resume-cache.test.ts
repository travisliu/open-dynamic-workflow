...
export default async (ctx) => {
  ...
  it("does not count cache hits as new live agent calls during resume", async () => {
    const workflowPath = path.join(TEMP_DIR, "workflows/budgeted-resume.workflow.ts");
    const configPath = path.join(TEMP_DIR, "budgeted-resume.config.yaml");
    const runsDir = path.join(TEMP_DIR, "budgeted-resume-runs");
    const counterPath = path.join(TEMP_DIR, "budgeted-resume-counter.txt");
    await writeConfig(configPath);
    await fs.writeFile(workflowPath, `export const meta = { name: "budgeted-resume", description: "budgeted resume test" };
export default async (ctx) => {
