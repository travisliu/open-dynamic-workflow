...
describe("openflow init integration", () => {
  let tmpDir: string;

  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "openflow-init-test-"));
  ...
  it("creates default project structure", async () => {
    ...
    expect(fs.existsSync(path.join(tmpDir, ".openflow/config.yaml"))).toBe(true);
  ...
  it("respects custom directory flags", async () => {
    ...
    const config = fs.readFileSync(path.join(tmpDir, ".openflow/config.yaml"), "utf8");
  ...
  it("fails before writing when .openflow/agents is a file", async () => {
    fs.mkdirSync(path.join(tmpDir, ".openflow"), { recursive: true });
    fs.writeFileSync(path.join(tmpDir, ".openflow/agents"), "I am a file");

    await expect(runInit()).rejects.toThrow(/Cannot reuse "\.openflow\/agents" as a directory because it is a file/);
    expect(fs.existsSync(path.join(tmpDir, ".openflow/config.yaml"))).toBe(false);
  });
  ...
  it("fails before writing when .openflow is a file (unplanned parent)", async () => {
    fs.writeFileSync(path.join(tmpDir, ".openflow"), "I am a file");

    await expect(runInit()).rejects.toThrow(/Cannot create "\.openflow\/config\.yaml" because parent path "\.openflow" is a file, not a directory/);
    expect(fs.readFileSync(path.join(tmpDir, ".openflow"), "utf8")).toBe("I am a file");
  ...
  it("moves summary to stderr and keeps stdout clean for JSON report", async () => {
    ...
    expect(stdoutData).not.toContain("OpenFlow project initialized");
    
    // summary should be on stderr
    expect(stderrData).toContain("OpenFlow project initialized");
  });
  ...
  it("skips existing files and reports them in summary", async () => {
    fs.mkdirSync(path.join(tmpDir, ".openflow"), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, "workflows"), { recursive: true });
    fs.writeFileSync(path.join(tmpDir, ".openflow/config.yaml"), "sentinel config");
    fs.writeFileSync(path.join(tmpDir, "workflows/example.ts"), "sentinel workflow");

    const { stdoutData } = await runInit();

    expect(fs.readFileSync(path.join(tmpDir, ".openflow/config.yaml"), "utf8")).toBe("sentinel config");
    ...
    expect(stdoutData).toContain(".openflow/config.yaml");
  ...
  it("lists all conflicts in strict mode", async () => {
    fs.mkdirSync(path.join(tmpDir, ".openflow"), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, "workflows"), { recursive: true });
    fs.writeFileSync(path.join(tmpDir, ".openflow/config.yaml"), "existing config");
    ...
    expect(error.stdoutData).toContain(".openflow/config.yaml");
    expect(error.stdoutData).toContain("workflows/example.ts");
    
    // Check if missing targets were NOT created
    expect(fs.existsSync(path.join(tmpDir, ".openflow/agents"))).toBe(false);
  });
});
